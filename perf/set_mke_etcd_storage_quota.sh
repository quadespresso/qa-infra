...
ETCD_SIZE_IN_GB_MIN=1
ETCD_SIZE_IN_GB_MAX=32
ETCD_SIZE_IN_GB_DEFAULT=2
ETCD_SIZE_IN_GB="${ETCD_SIZE_IN_GB_DEFAULT}GB"
...
Updates the etcd storage size limit
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -g|--size-in-gb)
            if [[ ! "$2" =~ ^[0-9]+$ ]]; then
                printf "Error: The etcd size must be a valid integer after --size-in-gb.\n"
                exit 1
            fi
            if (( $2 < $ETCD_SIZE_IN_GB_MIN || $2 > $ETCD_SIZE_IN_GB_MAX )); then
                printf "Error: The etcd size must be between $ETCD_SIZE_IN_GB_MIN and $ETCD_SIZE_IN_GB_MAX.\n"
                exit 1
            fi
            # ETCD_SIZE_IN_GB="$2"
            ETCD_SIZE_IN_GB="${2}GB"
            shift 2
            ;;
        -v|--view)
            VIEW_ONLY=true
...
if [ "$ETCD_SIZE_IN_GB_SETTING" == "$ETCD_SIZE_IN_GB" ]; then
    printf "The MKE etcd size already matches the desired value - no update performed.\n"
    exit 0
fi
printf "Updating MKE etcd size from [$ETCD_SIZE_IN_GB_SETTING] to [$ETCD_SIZE_IN_GB]...\n"
sed -i -e "/etcd_storage_quota =/ s/= .*/= \"$ETCD_SIZE_IN_GB\"/" $MKE_CONFIG_TOML_PATH
upload_mke_config_toml "$MKE_HOST" "$AUTHTOKEN" "$MKE_CONFIG_TOML_PATH"
if [ $? -ne 0 ]; then
    exit 1
fi
printf "Updating MKE etcd size complete.\n"
